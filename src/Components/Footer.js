...
const Glass=styled.div`align-self:end;padding:30px;background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.25);backdrop-filter:blur(18px);font-size:17px;line-height:1.7`;
const Action=styled(Link)`display:flex;align-items:center;justify-content:space-between;margin-top:25px;padding:18px 0;border-top:1px solid rgba(255,255,255,.3);font-size:14px;font-weight:700;text-transform:uppercase;letter-spacing:.1em`;
...
const copy={es:{title:"Tu próximo espacio empieza con una conversación.",text:"Cerramientos y protección solar diseñados con precisión para una vida más luminosa, eficiente y conectada.",area:"Cataluña"},ca:{title:"El teu pròxim espai comença amb una conversa.",text:"Tancaments i protecció solar dissenyats amb precisió per a una vida més lluminosa, eficient i connectada.",area:"Catalunya"},en:{title:"Your next space starts with a conversation.",text:"Precisely designed windows and solar protection for a brighter, more efficient and connected life.",area:"Catalonia"}};
export default function Footer(){const {t,lang}=useLanguage();const c=copy[lang]||copy.es;return <Foot><Inner><Top><div><Kicker>Finestra Serveis · Barcelona</Kicker><H2>{c.title}</H2></div><Glass>{c.text}<Action to="/contacto">{t("quote")} <FiArrowUpRight/></Action></Glass></Top></Inner>
<Bottom><span>© {new Date().getFullYear()} Finestra Serveis</span><Contact><a href="tel:+34691292245"><FiPhone/> +34 691 292 245</a><span><FiMapPin/> {c.area}</span></Contact><Social><a href="https://instagram.com/finestra.serveis" aria-label="Instagram"><FaInstagram/></a><a href="https://wa.me/34691292245?text=Hola%2C%20escribo%20desde%20la%20p%C3%A1gina%20web%20de%20Finestra%20Serveis.%20Quisiera%20solicitar%20informaci%C3%B3n%20sobre%20un%20proyecto." aria-label="WhatsApp"><FaWhatsapp/></a></Social></Bottom></Foot>}
